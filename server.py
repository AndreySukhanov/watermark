...
def _build_quality_analysis(body: dict) -> dict:
    ...
    if not input_path:
        raise RuntimeError("Missing video path")
    ...
    if not base_regions:
        raise RuntimeError("Quality analysis requires at least one region")
    ...
    try:
        ...
        if config.family == "propainter" and config.propainter_use_crops:
            ...
            planned_groups = plan_propainter_crop_groups(width, height, crop_regions, config)
            if planned_groups:
...
def _run_lama_pipeline(
    ...
    try:
        ...
        emit_log(
            f"Engine: {config.label}. Frames: {total_frames}. "
            f"Skip={skip} → process {to_process}. Parallelism: x{worker_count}."
        )
        ...
        for batch_start in range(0, total_frames, BATCH_SIZE):
            ...
            _raise_if_cancelled(job_id)
            emit_log(f"Batch {batch_num}: extracting up to {expected_batch_count} frames...")
            ...
            if actual_batch_count == 0:
                emit_log(f"  Batch {batch_num}: ffmpeg returned no frames, stopping at {processed} frames")
                break
            actual_total_frames = max(actual_total_frames, batch_start + actual_batch_count)
            if actual_batch_count != expected_batch_count:
                emit_log(
                    f"  Batch {batch_num}: extracted {actual_batch_count} instead of "
            ...
            emit_log(f"  Thin: {actual_batch_count} → {kept} frames")
            emit_log(f"  Thin: {time.perf_counter() - thin_started_at:.1f}s")

            _raise_if_cancelled(job_id)
            emit_log(f"Batch {batch_num}: IOPaint x{worker_count} ({device})...")
            ...
            _raise_if_cancelled(job_id)
            if not success:
                raise RuntimeError(err or "IOPaint exited with error")
            emit_log(f"  IOPaint: {time.perf_counter() - iopaint_started_at:.1f}s")

            _raise_if_cancelled(job_id)
            emit_log(f"Batch {batch_num}: composing final frames...")
            ...
            if written != actual_batch_count:
                raise RuntimeError(
                    f"Failed to compose batch {batch_num}: "
                    f"expected {actual_batch_count} frames, got {written}"
                )
            ...
            emit_log(
                f"  Batch {batch_num} composed ({done_so_far}/"
                f"{actual_total_frames or total_frames} frames)"
            )
        ...
        emit_log(f"Done in {time.perf_counter() - pipeline_started_at:.1f}s")
...
def _run_propainter_quality(
    ...
    try:
        ...
        emit_log(f"Done in {time.perf_counter() - started_at:.1f}s")
    except subprocess.CalledProcessError as e:
        details = (e.stderr or e.output or "").strip()
        raise RuntimeError(details or str(e)) from e
    finally:
...
async def extract_frame(path: str = Query(...), time: float = 1.0):
    ...
    return JSONResponse({"error": "Failed to extract frame"}, status_code=500)
...
async def _process_delogo(ws, params, regions, out_path, job_id):
    ...
    await ws.send_json({"type": "log", "data": "Removing watermark..."})
    ...
    if returncode == 0:
        ...
        await ws.send_json({"type": "done", "success": False, "message": "Failed to process video. Please check the file is valid."})
...
async def _process_ai(ws, params, regions, out_path, job_id, device="cpu"):
    ...
    while True:
        ...
        if kind == "log":
            ...
            await ws.send_json({"type": "done", "success": False, "message": "Cancelled by user"})
            return
        elif kind == "error":
