...
function startProcessing() {
  ...
  ws.onerror = () => onError('Lost connection to server');
  ws.onclose = () => {
    if (state.status === 'processing') onError('Connection closed');
  };
}
...
function onDone(msg) {
  ...
  if (msg.success) {
    ...
    document.getElementById('done-subtitle').textContent =
      `Watermark removed in ${formatTimer(elapsed)}`;
