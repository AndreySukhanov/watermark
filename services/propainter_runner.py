...
def ensure_propainter_available(propainter_dir: Path | None = None):
    ...
    if not (repo_dir / "inference_propainter.py").exists():
        raise RuntimeError(
            "ProPainter is not installed. Expected repository at "
            f"{repo_dir}"
        )
    return repo_dir
...
def _run_propainter_inference(
    *,
    ...
    if not output_video.exists():
        raise RuntimeError(f"ProPainter output not found: {output_video}")
    return output_video
...
def _run_propainter_full_frame_pipeline(
    *,
    ...
    emit_log("ProPainter: extracting source frames...")
...
def _run_propainter_crop_pipeline(
    *,
    ...
    if not crop_groups:
        raise RuntimeError("Failed to build crop groups for ProPainter")
    ...
    for index, group in enumerate(crop_groups, start=1):
        ...
        emit_log(f"ProPainter crop {index}/{len(crop_groups)}: frames...")
        ...
        emit_log(f"ProPainter crop {index}/{len(crop_groups)}: extracting output...")
        ...
        emit_log(f"ProPainter crop {index}/{len(crop_groups)}: composing...")
    ...
    emit_log("ProPainter crop: reassembling video...")
    emit_progress(92)
...
def run_propainter_pipeline(
    ...
    if info is None:
        raise RuntimeError(f"Failed to read video: {input_video}")
